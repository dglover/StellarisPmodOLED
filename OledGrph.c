...
**
**	Description:
**		Return a pointer to the unsigned char array for the specified
...
**		pattern is an array of 8 unsigned chars.
*/
	...
	*/
	...
	while (ycoTop <= ycoBottom) {
		...
		*/
		...
		/* Generate a mask to preserve any low bits in the unsigned char that aren't
		** part of the rectangle being filled.
		*/
		mskPat = (1 << (ycoTop & 0x07)) - 1;

		/* Combine with a mask to preserve any upper bits in the unsigned char that aren't
		** part of the rectangle being filled.
		** This mask will end up not preserving any bits for unsigned chars that are in
		** the middle of the rectangle vertically.
		*/
		if ((ycoTop / 8) == (ycoBottom / 8)) {
			mskPat |= ~((1 << ((ycoBottom&0x07)+1)) - 1);
		}											
		ibPat = xcoLeft & 0x07;		//index to first pattern unsigned char
		xcoCur = xcoLeft;
		pbCur = pbLeft;

		/* Loop through all of the unsigned chars horizontally making up this stripe
		** of the rectangle.
		*/
...
OledMoveUp()
	...
	/* Go up one bit position in the current unsigned char.
	*/
	bnOledCur -= 1;

	/* If we have gone off the end of the current unsigned char
	** go up 1 page.
	*/
	if (bnOledCur < 0) {
		bnOledCur = 7;
...
OledMoveDown()
	...
	/* Go down one bit position in the current unsigned char.
	*/
	bnOledCur += 1;
...
OledMoveLeft()
	...
	*/
	...
	/* Not at the left edge, so go back one unsigned char.
	*/
	pbOledCur -= 1;
...
OledMoveRight()
	...
	*/
	...
	/* Not at the right edge, so go forward one unsigned char
	*/
	pbOledCur += 1;
